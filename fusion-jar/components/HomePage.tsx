...
export function HomePage() {
  ...
  const levelData = gamificationData ? calculateLevel(gamificationData.xp_points) : { level: 1, xpToNext: 10, progress: 0 };
  const totalInvested = recentInvestments.reduce((sum, inv) => sum + (inv.amount || 0), 0);
  const uniqueChains = new Set(recentInvestments.map(inv => inv.source_chain)).size;
  return (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          ...
          <div className="bg-gray-800 rounded-lg p-6 col-span-2">
            ...
            <div className="flex justify-between text-xs text-gray-400 mb-2">
              ...
              <span>{(gamificationData?.xp_points || 0) + levelData.xpToNext} XP</span>
            </div>
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <p className="text-blue-400 text-2xl font-bold">{uniqueChains || 0}</p>
          ...
          <div className="bg-gradient-to-br from-green-600/20 to-green-800/20 border border-green-500/30 rounded-xl p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-green-300 text-sm">Level {levelData.level + 1}</p>
              </div>
            </div>
            <div className="mb-3">
          ...
          <div className="bg-gradient-to-br from-yellow-600/20 to-yellow-800/20 border border-yellow-500/30 rounded-xl p-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-yellow-400 font-medium">{levelData.level}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm">Best Streak</span>
                <span className="text-yellow-400 font-medium">{gamificationData?.longest_streak || 0} days</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-300 text-sm">Total Investments</span>
                <span className="text-yellow-400 font-medium">{gamificationData?.total_investments || 0}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="bg-gray-800/50 rounded-lg p-8 text-center">
              ...
              <p className="text-gray-400 text-sm">
                Connect your wallet to see your investment history and track your DeFi journey.
              </p>
            </div>
            ...
            <div className="bg-gray-800/50 rounded-lg p-8 text-center">
              ...
              <h3 className="text-white text-lg font-medium mb-2">Start Your Journey</h3>
              <p className="text-gray-400 text-sm mb-4">
                You haven't made any investments yet. Start with as little as $1!
              </p>
              <button 
                onClick={() => window.location.href = '/create'}
            ...
            <div className="space-y-4">
              {recentInvestments.map((investment, index) => {
                ...
                return (
                  <div key={index} className="bg-gray-800 rounded-lg p-4 flex items-center justify-between hover:bg-gray-750 transition-colors">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 text-sm">
                          on {chainNames[investment.source_chain] || `Chain ${investment.source_chain}`}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
